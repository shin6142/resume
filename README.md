...
* 手戻りを防ぐための、PdM・PDとのE2Eテスト作成。
    ...
    考慮漏れによる実装の手戻りやテストケースの不足を防ぐため、実装前にE2Eテストの作成をPdMと実施した。Gauge/Playwrightを使用してマークダウン形式の自動E2Eスクリプトを作成することで、技術用語を使用せず受け入れ条件の認識を揃えることができた。
...
### 概要
保育料の計算機能をインボイス制度に対応をするため、消費税計算処理とその結果を表示するブラウザ・Excel/CSV出力・モバイルアプリの改修を担当した。
